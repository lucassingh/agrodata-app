...
export async function loadTenantCatalog(tenantId: string): Promise<TenantCatalog> {
  const [expenseCategories, supplyCategories, supplies, pastures, animalCategories] = await Promise.all([
  ...
  return { expenseCategories, supplyCategories, supplies, pastures, animalCategories };
}
